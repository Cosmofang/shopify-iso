...
| 3 | 新商家容易看到和找到 | 引导难以定位；官方明确举例包括折叠或出现在当前视野外 |
...
- Onboarding 必须容易定位。默认折叠或出现在当前视野外是官方点名的拒审风险；“首屏内、默认展开”是 ISO 的保守实现基线，不是 Shopify 公布的固定像素或视口阈值。
...
> “最多 5 步”、自动完成、进度指示和稍后继续来自官方体验指南，是强烈推荐做法，但没有单独列为 BFS 4.2.2 的拒审条目。六条硬性判据仍须全部通过。官方 Onboarding 页仍保留“没有现成 pattern”的旧句子，但同页已链接当前 Setup guide composition；实现时以当前 composition/API 为准。
...
| 关闭/折叠 | tertiary `s-button`，`icon="x"` / chevron | 提供准确 `accessibilityLabel`；不得让引导因此难以定位 |
| 步骤 | `s-checkbox` + heading/description | checkbox 反映真实完成状态，不要求商家手工“自证完成” |
| 步骤动作 | `s-button variant="primary"` | 直接进入该步任务；同一步的相关动作明确主次 |
...
| 状态 | 含义 | UI 行为 |
|---|---|---|
| `incomplete` | 必要任务尚未完成 | 容易定位；ISO 默认采用首页可视区域内展开；步骤状态来自真实 App 数据 |
...
- 新安装首页截图：guide 容易定位；采用 ISO 保守基线时在初始可视区域内展开。
